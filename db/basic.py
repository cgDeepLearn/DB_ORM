...
class myDataBase(object):
    ...
    def connect(self):
        ...
        if self._dbtype == 'sqlite':
            ...
            args = ('host','user','password','database','port','use_unicode','charset')
            values = (self._dbargs['host'], self._dbargs['user'],self._dbargs['password'],
                        self._dbargs['dbname'], int(self._dbargs['port']),'True','utf8')
            ...
            values = (self._dbargs['host'], self._dbargs['user'],self._dbargs['password'],
                        self._dbargs['dbname'],int(self._dbargs['port']))
            kwargs = dict(zip(args, values))
    ...
    def insert(self, sql):
        ...
        if self._conn:
            try:
                self._cursor.execute(sql)
                self._conn.commit()
                res = self._cursor.rowcount if hasattr(self._cursor, 'rowcount') else -1
